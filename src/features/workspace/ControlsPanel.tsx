...
import { Crop, Lasso, PenLine, Ruler, Wand2 } from 'lucide-react';
    ...
    return (
      <div>
        ...
        <Section title={t('params.image')}>
          <SubSection title={t('params.framing')}>
            <div className="flex items-center gap-3 py-1.5">
              <span className="w-24 shrink-0 text-sm text-slate-600">{t('params.rotate')}</span>
            ...
            <div className="flex items-center gap-3 py-1.5">
              <span className="w-24 shrink-0 text-sm text-slate-600">{t('params.cropTool')}</span>
        ...
        <Section title={t('tabs.outline')}>
          ...
          {/* Selection — how to grab the object: the magic (automatic) tool, or trace it by hand. */}
          <SubSection title={t('params.selection')}>
            <div className="mb-1 flex items-center gap-1.5 text-xs font-medium text-slate-500">
              <Wand2 size={13} /> {t('params.selAutomatic')}
            </div>
            <Tabs
              tabs={[
            ...
            />
            <div className="mb-1 mt-3 text-xs font-medium text-slate-500">{t('params.selManual')}</div>
            <button
              type="button"
              onClick={() => onFrameTool(frameTool === 'lasso' ? 'none' : 'lasso')}
              disabled={!canLasso}
              aria-pressed={frameTool === 'lasso'}
              className={`flex w-full items-center justify-center gap-1.5 rounded-lg border py-1.5 text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
                frameTool === 'lasso'
            ...
            {frameTool === 'lasso' && <p className="mt-1 text-xs leading-relaxed text-slate-400">{t('params.lassoHint')}</p>}
          </SubSection>

          <SubSection title={t('params.refine')}>
            {/* Editable contour (spec 035): hand-tune with draggable nodes. */}
            {editingContour ? (
              <div className="rounded-lg bg-accent-50 px-3 py-2 ring-1 ring-accent-200">
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-slate-200 py-1.5 text-sm text-slate-600 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
              >
            ...
            {hasManualContour && !editingContour && (
              <div className="mt-1 flex items-center justify-end gap-2">
                <span className="text-xs text-accent-600">{t('params.contourManual')}</span>
                <button
                  type="button"
                  onClick={onClearContour}
                  className="rounded-lg border border-slate-200 px-2 py-1 text-xs text-slate-500 transition-colors hover:bg-slate-50"
                >
                  {t('params.contourClear')}
                </button>
              </div>
            )}
            <div className="mb-1 mt-3 text-xs font-medium text-slate-500">{t('params.brush')}</div>
            <Tabs
              tabs={[
            ...
            <button
              ...
              disabled={!hasEdits}
              className="text-xs font-medium text-accent-700 hover:underline disabled:text-slate-300 disabled:no-underline"
            >
              {t('params.reset')}
            </button>
          </SubSection>

          {/* Advanced tuning + display aids — folded together, out of the way until needed. */}
          <Disclosure title={t('params.advanced')}>
            <Slider
            ...
            <Slider
              ...
              unit="mm"
            />
            <div className="mb-1 mt-3 text-xs font-medium text-slate-500">{t('params.display')}</div>
