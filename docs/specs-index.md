...
| 026 | [crop-rectangle](../specs/026-crop-rectangle/) | In review | **Crop tool redesign**: a `Crop`-icon button (no toggle) that proposes a red-dashed adjustable zone immediately — 8 resize handles + move + redraw, dimmed outside — applied once via **Appliquer** (one re-analysis, not live). Applying is **two-phase**: the cropped photo renders instantly (`framePhoto`), then the détourage runs and the contour appears when ready (`framed`/`framingPending` split, stale contour gated). Pure crop-box geometry unit-tested. Iteration 19. |
