...
| 026 | [crop-rectangle](../specs/026-crop-rectangle/) | In review | **Crop tool redesign**: a `Crop`-icon button (no toggle) that proposes a red-dashed adjustable zone immediately — 8 resize handles + move + redraw, dimmed outside — applied once via **Appliquer** (one re-analysis, not live). Pure crop-box geometry (`defaultCropBox`/`resizeCropBox`/`moveCropBox`) unit-tested. Iteration 19. |
