...
| 027 | [edge-segment](../specs/027-edge-segment/) | In review | **Détourage for transparent/reflective objects**: u2netp misses clear plastic (reads as background); add an **edge-based** mask (Canny → close → fill → largest blob) + a **Méthode** selector (Auto / Standard / Contours). **Auto** switches to edges only when u2netp clearly failed AND the edge silhouette is object-sized (guards textured backgrounds) — thresholds measured over the dataset; `chooseSegmentMode` unit-tested, edge pipeline covered by a tsx oracle. Iteration 20. |
