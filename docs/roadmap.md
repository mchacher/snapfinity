...
| 6 | **Revisit the calibration token?** | vision/calib + physical | R&D | Exploratory: more robust fiducial? different size? ArUco/AprilTag? **Decide before any spec.** Not urgent while detection holds. See #7 for the software-side detection tuning. |
| 7 | **Tunable / live token detection** | vision/calib | M | Today token detection runs **once per photo on the original gray** and is **cached** — none of the image sliders (flatten / brightness / contrast / threshold) affect it, and `tokenOdMm` only rescales mm/px from the found radius. So if the token isn't found, no control helps. Make it responsive: a **token-sensitivity** control (the `matchShapes` `maxScore` and/or the dark-threshold `darkCut` in `vision/token.ts`) with **live re-detection**, and optionally let brightness/contrast/flatten feed the token pass too (helps a token in shadow). Re-key the decode cache so detection re-runs on these. Pure-ish detection params are unit-testable on the dataset (token found / scale ± tol). |
